...
class Dashboard extends Component {
  ...
  async componentDidMount() {
    try {
      const employeesResponse = await fetch(`${process.env.REACT_APP_HOST}/employees/by-region/${this.state.region.id}`);
      const employees = await employeesResponse.json();
      const projectsResponse = await fetch(`${process.env.REACT_APP_HOST}/projects/by-region/${this.state.region.id}`)
  ...
  assignCrewMember = async (id, projectId) => {
    try {
      await fetch(`${process.env.REACT_APP_HOST}/employees/assign/${id}`, {
        method: 'PATCH',
  ...
  updateNotes = async (id) => {
    try {
      ...
      await fetch(`${process.env.REACT_APP_HOST}/projects/edit-notes/${id}`, {
        method: 'PATCH',
  ...
  toggleMilestone = async (id, milestoneName) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_HOST}/projects/toggle-milestones/${id}`, {
        method: 'PATCH',
